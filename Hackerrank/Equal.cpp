...
int main(){
    ...
    while(t--){
        ...
        for(int i=0; i<n; i++){
            int k=arr[i]-min1;
            moves[0]+=k/5 +(k%5)/2 +((k%5)%2);
        }
        for(int i=0; i<n; i++){
            int k=arr[i]-(min1-1);
            moves[1]+=k/5 +(k%5)/2 +((k%5)%2);
        }
        for(int i=0; i<n; i++){
            int k=arr[i]-(min1-2);
            moves[2]+=k/5 +(k%5)/2 +((k%5)%2);
        }
        for(int i=0; i<n; i++){
            int k=arr[i]-(min1-3);
            moves[3]+=k/5 +(k%5)/2 +((k%5)%2);
        }
        for(int i=0; i<n; i++){
            int k=arr[i]-(min1-4);
            moves[4]+=k/5 +(k%5)/2 +((k%5)%2);
        }
        cout<<*min_element(moves.begin(), moves.end())<<endl;
    }
    return 0;   
}
