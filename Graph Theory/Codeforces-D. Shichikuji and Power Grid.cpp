...
int main()
{
    ...
    for(ll i=1; i<=n; i++){
        for(ll j=i+1; j<=n; j++){
            ...
            ed.u=i;
            ed.v=j;
            ed.wght=(k[i]+k[j])*(abs(v[i].ff-v[j].ff)+abs(v[i].se-v[j].se));
            tu.pb(ed);
        }
    }
    par.resize(n+50);
    ...
    for(Edge edges: tu){
        if(find(edges.u)!=find(edges.v)){
            Union(edges.u, edges.v);
            cost+= edges.wght;
            if(edges.u==0)
                st.pb(edges.v);
            else
                conn.pb(mp(edges.u, edges.v));
        }
    }
    ...
    for(ll i=0; i<conn.size(); i++){
        if(conn[i].ff<conn[i].se)
            cout << conn[i].ff << " " << conn[i].se << endl;
        else
            cout << conn[i].se << " " << conn[i].ff << endl;
    }
    return 0;   
}
